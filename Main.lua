--[[
    Cyber Dragon | Rivals Cheat Script
...
local function checkExecutor()
    ...
    if #missing > 0 then
        ...
        game:GetService("Players").LocalPlayer:Kick(
            ...
            " — Use Solara, Delta, Wave, Synapse X, Xeno, Velocity, YuB-X, or Madium."
        )
        return false
    end
    return true
...
--------------------------------------------------------------------
-- 2. INITIALIZE SERVICES AND GLOBALS
--------------------------------------------------------------------
...
local farmPosition = "Behind"  -- "Behind", "Above", "Under"

--------------------------------------------------------------------
-- 3. MEMORY CACHING AND ATTRIBUTE MODIFICATION
--------------------------------------------------------------------
local cachedTables = {}
...
local function findAndCache(attribute)
    ...
    for _, gcVal in pairs(gc) do
        if type(gcVal) == "table" and rawget(gcVal, attribute) ~= nil then
            local original = gcVal[attribute]
            local typ = type(original)
            if typ == "number" or typ == "boolean" then
                table.insert(cachedTables[attribute], { tbl = gcVal, original = original, typ = typ })
            end
        end
    end
end
...
local function applyAttribute(attribute, value)
    ...
    for _, entry in pairs(cachedTables[attribute]) do
        if entry.tbl and (entry.typ == "number" or entry.typ == "boolean") then
...
local function restoreAttribute(attribute)
    ...
    for _, entry in pairs(cachedTables[attribute]) do
        if entry.tbl and entry.typ then
...
-- Generic cache for arbitrary keys
local function gcCacheOnce(keys)
    local cached = {}
    local success, gc = pcall(getgc, true)
    if not success then return cached end
    for _, gcVal in pairs(gc) do
        if type(gcVal) == "table" then
            for _, key in pairs(keys) do
                local original = rawget(gcVal, key)
                if original ~= nil then
                    local typ = type(original)
                    if typ == "number" or typ == "boolean" then
                        table.insert(cached, { tbl = gcVal, key = key, original = original, typ = typ })
                    end
                end
            end
        end
...
local function modifyCached(cached, value)
    for _, e in pairs(cached) do
        if e.tbl and (e.typ == "number" or e.typ == "boolean") then
            pcall(function() e.tbl[e.key] = value end)
        end
    end
end

local function restoreCached(cached)
    for _, e in pairs(cached) do
        if e.tbl and e.typ then
            pcall(function() e.tbl[e.key] = e.original end)
        end
    end
end

--------------------------------------------------------------------
-- 4. FEATURE FUNCTIONS (TOGGLES)
--------------------------------------------------------------------
-- Auto Farm
...
local function enableAutoFarm()
    autoFarmConn = RunService.Heartbeat:Connect(function()
        ...
        if targetHrp.Position.Magnitude > 10000 or targetHrp.Position.Y > 2000 then return end
...
local function enableFly()
    ...
    flyBodyVel.MaxForce = Vector3.new(1000000, 1000000, 1000000)
    flyBodyVel.P = 10000
    flyBodyVel.Parent = hrp
    flyBodyGyro = Instance.new("BodyGyro")
    flyBodyGyro.MaxTorque = Vector3.new(1000000, 1000000, 1000000)
    flyBodyGyro.P = 10000
    flyBodyGyro.D = 100
...
local function enableAntiKatana()
    katanaCached = gcCacheOnce({
        "ReflectBullets", "CanReflect", "IsBlocking", "KatanaBlocking",
        "KatanaActive", "ParryActive", "BulletReflect", "Deflect",
        "IsParrying", "KatanaReflect", "ReflectDamage"
    })
    modifyCached(katanaCached, false)
...
local function enableNoBounds()
    noBoundsCached = gcCacheOnce({
        "OutOfBounds", "IsOutOfBounds", "OOB", "outOfBounds",
        "BoundsDead", "VoidKill", "InVoid", "FellOff",
        "KillOnFall", "DeathFloor", "KillFloor"
    })
    modifyCached(noBoundsCached, false)
    local char = plr.Character
    if char then
        local hrp = char:FindFirstChild("HumanoidRootPart")
    ...
    noBoundsConn = RunService.Heartbeat:Connect(function()
        ...
        if hrp.Position.Y < -80 and lastSafePos then
...
local function enableRapidFire()
    ...
    modifyCached(rapidFireCached, 0)
end

local function disableRapidFire()
    restoreCached(rapidFireCached)
    rapidFireCached = {}
end
...
local function enableAutoWeapon()
    ...
    modifyCached(autoWeaponCached, true)
end

local function disableAutoWeapon()
    restoreCached(autoWeaponCached)
    autoWeaponCached = {}
end
...
local function enableInstantScope()
    ...
    modifyCached(instantScopeCached, 0)
end

local function disableInstantScope()
    restoreCached(instantScopeCached)
    instantScopeCached = {}
end
...
local function enableAlwaysBackstab()
    ...
    for _, e in pairs(backstabCached) do
        if e.typ == "boolean" then
            pcall(function() e.tbl[e.key] = true end)
        elseif e.typ == "number" then
            pcall(function() e.tbl[e.key] = 360 end)
        end
    end
...
local function disableRemoveKillers()
    for _, d in pairs(removedKillers) do
        pcall(function() d.part.CanTouch = d.canTouch end)
    end
    removedKillers = {}
end
...
local function enableNoFireDamage()
    ...
    modifyCached(fireDamageCached, 0)
...
local function disableNoFireDamage()
    ...
    restoreCached(fireDamageCached)
    fireDamageCached = {}
end
...
RunService.Heartbeat:Connect(function()
    ...
    local now = tick()
    if now - lastRun < 0.05 then return end
    lastRun = now
    local char = plr.Character
    if not char then return end
    local hrp = char:FindFirstChild("HumanoidRootPart")
...
local function showHitMarker()
    ...
    TweenService:Create(hitMarker, TweenInfo.new(0.08, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { BackgroundTransparency = 1 }):Play()
    task.delay(0.12, function() if hitMarker then hitMarker.Visible = false end end)
    pulse.Visible = true
    pulse.Size = UDim2.new(0, 0, 0, 0)
    pulse.BackgroundTransparency = 0.7
    TweenService:Create(pulse, TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.new(0, 120, 0, 120), BackgroundTransparency = 1 }):Play()
    task.delay(0.2, function() if pulse then pulse.Visible = false end end)
end

local DAMAGE_LIFESPAN = 1
local DAMAGE_RISE_SPEED = 40
local DAMAGE_WAVE_AMPLITUDE = 12
local DAMAGE_WAVE_FREQUENCY = 3
...
local function showDamageNumber(enemyChar, damage, isHeadshot)
    ...
    connection = RunService.RenderStepped:Connect(function()
        ...
        local offsetX = math.sin(angle) * DAMAGE_WAVE_AMPLITUDE * (1 - t)
...
local function createESP(player)
    if espObjects[player] or not drawingSupported then return end
    espObjects[player] = {
...
local function updateESP(player)
    ...
    local b = getCharacterBounds(char)
    if not b then hideESP(player); return end
    esp.box.Position = Vector2.new(b.x, b.y)
    esp.box.Size = Vector2.new(b.w, b.h)
    esp.box.Visible = true
    esp.boxFill.Position = Vector2.new(b.x, b.y)
    esp.boxFill.Size = Vector2.new(b.w, b.h)
    esp.boxFill.Visible = true
    esp.name.Text = player.DisplayName
    esp.name.Position = Vector2.new(b.cx, b.y - 16)
    esp.name.Visible = true
    esp.distance.Text = math.floor(dist) .. "m"
    esp.distance.Position = Vector2.new(b.cx, b.y + b.h + 2)
    esp.distance.Visible = true
    local hp = math.clamp(hum.Health / math.max(hum.MaxHealth, 1), 0, 1)
    local barH = b.h * hp
    esp.healthBar.Color = Color3.new(math.clamp(2 * (1 - hp), 0, 1), math.clamp(2 * hp, 0, 1), 0)
    esp.healthBg.Position = Vector2.new(b.x - 7, b.y)
    esp.healthBg.Size = Vector2.new(4, b.h)
    esp.healthBg.Visible = true
    esp.healthBar.Position = Vector2.new(b.x - 7, (b.y + b.h) - barH)
...
RunService.RenderStepped:Connect(function()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= plr then
            if state.ESP then updateESP(p) else hideESP(p) end
        end
    end
end)
...
closeBtn.MouseButton1Click:Connect(function()
    ...
    TweenService:Create(root, TweenInfo.new(0.2, Enum.EasingStyle.Quad), {
        Size = minimized and UDim2.new(0, rootW, 0, titleBarH) or UDim2.new(0, rootW, 0, rootH)
    }):Play()
        ...
        if input.KeyCode == Enum.KeyCode.RightShift then
            ...
            if hidden then
                TweenService:Create(root, TweenInfo.new(0.25, Enum.EasingStyle.Quad), {
                    Position = UDim2.new(0, -560, 0.5, -190),
                ...
                TweenService:Create(root, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
                    Position = UDim2.new(rootPosX, rootPosXO, rootPosY, rootPosYO),
...
local function makeToggle(parent, labelText, key, order, onEnable, onDisable)
    ...
    local function updateVisual(on)
        TweenService:Create(pillBg, TweenInfo.new(0.18), {
            BackgroundColor3 = on and Color3.fromRGB(0, 35, 28) or C.bg3
        }):Play()
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(row, TweenInfo.new(0.12), { BackgroundColor3 = Color3.fromRGB(18, 22, 28) }):Play()
    end)
    btn.MouseLeave:Connect(function()
        TweenService:Create(row, TweenInfo.new(0.12), { BackgroundColor3 = C.bg2 }):Play()
    end)
    btn.MouseButton1Down:Connect(function()
        TweenService:Create(row, TweenInfo.new(0.08), { BackgroundColor3 = Color3.fromRGB(0, 25, 20) }):Play()
    end)
    btn.MouseButton1Up:Connect(function()
        TweenService:Create(row, TweenInfo.new(0.08), { BackgroundColor3 = Color3.fromRGB(18, 22, 28) }):Play()
    end)
    updateVisual(state[key])
end
...
local sL, sR = createTab("Settings", 5)
local scriptStartTime = tick()
local usageLabel = nil
local function createLabel(text, sizeY)
    local lbl = Instance.new("TextLabel")
    lbl.Size = UDim2.new(1, -20, 0, sizeY or 30)
    lbl.BackgroundTransparency = 1
    lbl.Text = text
    lbl.TextColor3 = C.text
    lbl.TextSize = 13
...
headerLeft.Size = UDim2.new(1, -20, 0, 25)
headerLeft.BackgroundTransparency = 1
headerLeft.Text = "▸ SCRIPT INFORMATION"
headerLeft.TextColor3 = C.neon
headerLeft.TextSize = 12
headerLeft.Font = Enum.Font.GothamBold
headerLeft.TextXAlignment = Enum.TextXAlignment.Left
headerLeft.Parent = sL
local lastUpdateLabel = createLabel("📅 Last Update: April 25, 2025", 25)
local detectionLabel = Instance.new("TextLabel")
detectionLabel.Size = UDim2.new(1, -20, 0, 30)
detectionLabel.BackgroundTransparency = 1
detectionLabel.Text = "🟢 Status: Undetected"
detectionLabel.TextColor3 = Color3.fromRGB(0, 255, 100)
detectionLabel.TextSize = 13
detectionLabel.Font = Enum.Font.GothamBold
detectionLabel.TextXAlignment = Enum.TextXAlignment.Left
detectionLabel.Parent = sL
usageLabel = createLabel("⏱️ Usage Time: 0 seconds", 30)
local spacer = Instance.new("Frame")
spacer.Size = UDim2.new(1, 0, 0, 10)
spacer.BackgroundTransparency = 1
spacer.Parent = sL
local headerRight = Instance.new("TextLabel")
headerRight.Size = UDim2.new(1, -20, 0, 25)
headerRight.BackgroundTransparency = 1
headerRight.Text = "▸ NOTES"
headerRight.TextColor3 = C.neon
headerRight.TextSize = 12
headerRight.Font = Enum.Font.GothamBold
headerRight.TextXAlignment = Enum.TextXAlignment.Left
headerRight.Parent = sR
local function createOnRight(text, sizeY)
    local lbl = Instance.new("TextLabel")
    lbl.Size = UDim2.new(1, -20, 0, sizeY or 25)
    lbl.BackgroundTransparency = 1
    lbl.Text = text
    lbl.TextColor3 = C.text
    lbl.TextSize = 12
...
createOnRight("• Skins are client‑side only – only you see them.", 25)
createOnRight("• Press RightShift to toggle the menu.", 25)
createOnRight("• Configs are saved automatically.", 25)
createOnRight("• Report issues to the developer.", 25)
...
task.spawn(function()
    while true do
        ...
        if hours > 0 or days > 0 then timeStr = timeStr .. hours .. "h " end
        if minutes > 0 or hours > 0 or days > 0 then timeStr = timeStr .. minutes .. "m " end
        timeStr = timeStr .. seconds .. "s"
        usageLabel.Text = "⏱️ Usage Time: " .. timeStr
    end
end)
...
--------------------------------------------------------------------
-- 7. SKIN CHANGER (Heavily refactored but same functionality)
--------------------------------------------------------------------
local SkinLists = {
    ["Assault Rifle"] = { "Default", "AK-47", "AUG", "Tommy Gun", "Boneclaw Rifle", "Gingerbread AUG", "AKEY-47", "100K Visits", "10 Billion Visits", "Phoenix Rifle", "Neon Rider", "Galactic", "Magma Core" },
    Bow = { "Default", "Compound Bow", "Raven Bow", "Dream Bow", "Bat Bow", "Frostbite Bow", "Beloved Bow", "Balloon Bow", "Glorious Bow", "Key Bow", "Arch Bow", "Prism Bow", "Void Bow" },
    ["Burst Rifle"] = { "Default", "Electro Burst", "Aqua Burst", "FAMAS", "Spectral Burst", "Pine Burst", "Cyber Burst", "Magma Burst" },
    Crossbow = { "Default", "Pixel Crossbow", "Harpoon Crossbow", "Violin Crossbow", "Crossbone", "Frostbite Crossbow", "Arch Crossbow", "Glorious Crossbow", "Railgun Crossbow", "Plasma Crossbow" },
    Distortion = { "Default", "Plasma Distortion", "Magma Distortion", "Cyber Distortion", "Expirement D15", "Sleighstortion", "Void Distortion" },
    ["Energy Rifle"] = { "Default", "Hacker Rifle", "Hydro Rifle", "Void Rifle", "Soul Rifle", "New Years Energy Rifle", "Plasma Rifle", "Neon Rifle" },
    Flamethrower = { "Default", "Pixel Flamethrower", "Lamethrower", "Glitterthrower", "Jack O' Thrower", "Snowblower", "Keythrower", "Rainbowthrower", "Dragonfire", "Extinguisher" },
    ["Grenade Launcher"] = { "Default", "Swashbuckler", "Uranium Launcher", "Gearnade Launcher", "Skull Grenade Launcher", "Snowball Launcher", "Cluster Launcher" },
    Gunblade = { "Default", "Hyper Gunblade", "Crude Gunblade", "Gunsaw", "Boneblade", "Elf's Gunblade", "Void Blade", "Keyblade" },
    Minigun = { "Default", "Lasergun 3000", "Pixel Minigun", "Fighter Jet", "Pumpkin Minigun", "Wrapped Minigun", "Gatling Plasma" },
    ["Paintball Gun"] = { "Default", "Slime Gun", "Boba Gun", "Ketchup Gun", "Brain Gun", "Snowball Gun", "Rainbow Splatter" },
    RPG = { "Default", "Nuke Launcher", "Spaceship Launcher", "Squid Launcher", "Pumpkin Launcher", "Firework Launcher", "Meteor Launcher" },
    Shotgun = { "Default", "Balloon Shotgun", "Hyper Shotgun", "Cactus Shotgun", "Broomstick", "Wrapped Shotgun", "Dragon Breath", "Tactical Shotgun" },
    Sniper = { "Default", "Pixel Sniper", "Hyper Sniper", "Event Horizon", "Eyething Sniper", "Gingerbread Sniper", "Keyper", "Glorious Sniper", "Railgun Sniper", "Plasma Sniper" },
    Daggers = { "Default", "Aces", "Paper Planes", "Shurikens", "Bat Daggers", "Cookies", "Crystal Daggers", "Keynais", "Void Daggers", "Toaster" },
    ["Energy Pistols"] = { "Default", "Void Pistols", "Hydro Pistols", "Soul Pistols", "New Years Energy Pistols", "Plasma Pistols" },
    Exogun = { "Default", "Singularity", "Raygun", "Repulsor", "Exogourd", "Midnight Festive Exogun", "Neon Exogun" },
    ["Flare Gun"] = { "Default", "Firework Gun", "Dynamite Gun", "Banana Flare", "Vexed Flare Gun", "Wrapped Flare Gun", "Solar Flare" },
    Handgun = { "Default", "Blaster", "Hand Gun", "Stealth Handgun", "Pumpkin Handgun", "Gingerbread Handgun", "Magnum" },
    Revolver = { "Default", "Desert Eagle", "Sheriff", "Peppergun", "Boneclaw Revolver", "Peppermint Sheriff", "Python" },
    Shorty = { "Default", "Not So Shorty", "Lovely Shorty", "Balloon Shorty", "Demon Shorty", "Wrapped Shorty", "Breacher" },
    Slingshot = { "Default", "Stick", "Goal Post", "Harp", "Boneshot", "Reindeer Slingshot", "Lucky Horseshoe", "Galactic Slingshot", "Keyshot" },
    Spray = { "Default", "Lovely Spray", "Nail Gun", "Bottle Spray", "Boneclaw Spray", "Pine Spray", "Key Spray", "Neon Spray" },
    Uzi = { "Default", "Water Uzi", "Electro Uzi", "Money Gun", "Demon Uzi", "Pine Uzi", "Plasma Uzi" },
    Warper = { "Default", "Glitter Warper", "Arcane Warper", "Hotel Bell", "Experiment W4", "Frost Warper", "Void Warper" },
    ["Battle Axe"] = { "Default", "The Shred", "Ban Axe", "Cerulean Axe", "Mimic Axe", "Nordic Axe", "Dragon Axe", "Street Sign" },
    Chainsaw = { "Default", "Blobsaw", "Handsaws", "Mega Drill", "Buzzsaw", "Festive Buzzsaw", "Ripper" },
    Fists = { "Default", "Boxing Gloves", "Brass Knuckles", "Fists Of Hurt", "Pumpkin Claws", "Festive Fists", "Dragon Fists" },
    Katana = { "Default", "Saber", "Lightning Bolt", "Stellar Katana", "Evil Trident", "New Years Katana", "Keytana", "Arch Katana", "Crystal Katana", "Pixel Katana", "Glorious Katana", "Void Katana", "Plasma Katana" },
    Knife = { "Default", "Chancla", "Karambit", "Balisong", "Armature.001", "Candy Cane", "Keylisong", "Keyrambit", "Caladbolg", "Ghost Blade", "Pencil" },
    ["Riot Shield"] = { "Default", "Door", "Energy Shield", "Masterpiece", "Tombstone Shield", "Sled", "Bulwark" },
    Scythe = { "Default", "Scythe of Death", "Anchor", "Sakura Scythe", "Bat Scythe", "Cryo Scythe", "Crystal Scythe", "Keythe", "Bug Net", "Arch Scythe", "Reaper's Scythe" },
    Trowel = { "Default", "Plastic Shovel", "Garden Shovel", "Paintbrush", "Pumpkin Carver", "Snow Shovel", "Builder's Trowel" },
    Flashbang = { "Default", "Disco Ball", "Camera", "Lightbulb", "Skullbang", "Shining Star", "Solar Flare" },
    ["Freeze Ray"] = { "Default", "Temporal Ray", "Bubble Ray", "Gum Ray", "Spider Ray", "Wrapped Freeze Ray", "Cryo Cannon" },
    Grenade = { "Default", "Whoopee Cushion", "Water Balloon", "Dynamite", "Soul Grenade", "Jingle Grenade", "Frag Grenade" },
    ["Jump Pad"] = { "Default", "Trampoline", "Bounce House", "Shady Chicken Sandwich", "Spider Web", "Jolly Man", "Spring Pad" },
    Medkit = { "Default", "Sandwich", "Laptop", "Medkitty", "Bucket of Candy", "Milk & Cookies", "Box of Chocolates", "Briefcase", "First Aid" },
    Molotov = { "Default", "Coffee", "Torch", "Lava Lamp", "Vexed Candle", "Hot Coals", "Arch Molotov", "Incendiary" },
    Satchel = { "Default", "Advanced Satchel", "Notebook Satchel", "Bag O' Money", "Potion Satchel", "Suspicious Gift", "C4 Satchel", "Pizza Box" },
    ["Smoke Grenade"] = { "Default", "Emoji Cloud", "Balance", "Hourglass", "Eyeball", "Snowglobe", "Tactical Smoke" },
    ["Subspace Tripmine"] = { "Default", "Don't Press", "Spring", "DIY Tripmine", "Trick or Treat", "Dev In the Box", "Pot O Keys", "Proximity Mine" },
    ["War Horn"] = { "Default", "Trumpet", "Megaphone", "Air Horn", "Boneclaw Horn", "Mammoth Horn", "Battle Horn" },
    Warpstone = { "Default", "Cyber Warpstone", "Teleport Disc", "Electropunk Warpstone", "Warpbone", "Warpstar", "Void Stone" },
    Permafrost = { "Default", "Snowman Permafrost", "Ice Permafrost", "Glorious Permafrost", "Frozen Heart" },
    Maul = { "Default", "Ban Hammer", "Ice Maul", "Sleigh Maul", "Glorious Maul", "Thunder Maul" },
    Spear = { "Default", "Giant Pencil", "Studio Light", "Glorious Spear" },
    Grappler = { "Default", "Glorious Grappler", "Lasso" }
}

local WrapList = {
    "None", "Gold", "Diamond", "Midas Touch", "Community Wrap", "Blush Wrapping", "Brain", "Crystalliz", "Damascus", "Black Damascus",
    ".exe wrap", "Groove", "Hollow Wrap", "Hesper", "Hyperdrive", "Gingerbread", "Neon Lights", "Hologram Arena", "Sunset", "Pink Lemonade",
    "Lovely Leopard", "Dawn", "Spectral", "Danger", "Termination", "Moonstone", "Starfall", "Black Glass", "Rift Wrap", "Starblaze",
    "Maganite", "Watermelon", "Reptile", "Water", "OranGG", "A5", "Cheese", "Nova", "Supernova", "Glass", "Mesh", "Meat Wrap",
    "Black Dark Wrap", "Cardinal", "Pixel Camo", "Nauseite", "Sensite", "Urban Camo", "Invisible", "Arcane", "Eruption", "Borealis",
    "Mainframe Wrap", "Honeycomb Wrap", "Virus", "Patriot", "PB&J Wrap", "Scribble", "Net", "Solar", "Festive Lights", "Polaris",
    "Woven", "Heartfelt", "Chromatic", "Dark Matter", "Dev Wrap", "Admin Wrap", "Staff Wrap", "Shedletsky Wrap", "Telamon Wrap",
    "Badimo Wrap", "Mango Wrap", "Lava Wrap", "Developer", "Intern Wrap", "Alpha Tester", "Beta Tester", "Founder Wrap", "Creator Wrap",
    "Scripter Wrap", "Builder Wrap", "Season 1 Wrap", "Season 2 Wrap", "Season 3 Wrap", "Season 4 Wrap", "Season 5 Wrap", "Season 6 Wrap",
    "Contract Wrap I", "Contract Wrap II", "Contract Wrap III", "Contract Wrap IV", "Contract Wrap V", "Bronze Contract", "Silver Contract",
    "Gold Contract", "Platinum Contract", "Diamond Contract", "Elite Contract", "Champion Contract", "Prestige Wrap", "Legacy Wrap",
    "Veteran Wrap", "Anniversary Wrap", "Rivals Pass Wrap", "Battle Wrap", "Combat Wrap", "Victory Wrap", "Conquest Wrap", "Warpath Wrap",
    "Dominion Wrap", "Ascendant Wrap", "Neon Pulse", "Void Energy", "Plasma Stream", "Crystal Shard", "Galactic Swirl", "Magma Core",
    "Frostbite", "Radioactive", "Holographic", "Chroma Shift", "Dark Matter 2.0", "Sunset Gradient", "Midnight Sparkle", "Rainbow Prism",
    "Crime Scene", "Quartz", "Orichalcum", "Mossy", "Galvanized", "TV Error"
}

local CharmList = {
    "None", "Rivals Logo", "Sword", "Shield", "Crown", "Star", "Heart", "Skull", "Key", "Flame", "Ice Crystal", "Lightning", "Cherry Blossom",
    "Pumpkin", "Snowflake", "Rainbow", "Diamond Charm", "Ruby", "Emerald", "Sapphire", "Amethyst", "Gold Coin", "Dice", "Clover", "Mushroom",
    "Flower", "Butterfly", "Dragon", "Phoenix", "Wolf", "Cat", "Bunny", "Frog", "Duck", "Jellyfish", "Crab", "Anchor", "Compass", "Lantern",
    "Gem", "Pearl", "Feather", "Rose", "Sunflower", "Cactus", "Sword Charm", "Dagger Charm", "Axe Charm", "Bow Charm", "Shield Charm", "Bomb",
    "Rocket", "UFO", "Planet", "Moon", "Sun", "Cloud", "Rainbow Star", "Halloween Charm", "Christmas Charm", "Easter Charm", "Valentine Charm",
    "New Year Charm", "Summer Charm", "Winter Charm", "Spring Charm", "Fall Charm", "Common Charm", "Uncommon Charm", "Rare Charm", "Epic Charm",
    "Legendary Charm", "Mythical Charm", "Dev Charm", "Admin Charm", "Staff Charm", "Founder Charm", "Alpha Charm", "Beta Charm", "Season 1 Charm",
    "Season 2 Charm", "Season 3 Charm", "Season 4 Charm", "Season 5 Charm", "Contract Charm", "Victory Charm", "Champion Charm", "Prestige Charm",
    "Neon Skull", "Plasma Orb", "Void Core", "Frost Dragon", "Magma Heart", "Galactic Compass", "Holographic Eye", "Chronosphere", "Rift Stone"
}

local FinisherList = {
    "None", "Ragdoll", "Folded", "Very Tragic Banana Peel Accident", "Chalked", "Digitize", "Squawk", "Delete", "Flop", "Confetti", "Hacked",
    "Petrify", "Toot", "Yoink", "Spooky Confetti", "Bite", "Batsplosion", "Festive Confetti", "Wrapped", "Blip", "Chill Out", "Coalify",
    "Faceplant", "For Glory", "Northern Light Show", "Warped Away", "Hooked", "Bad Mood", "Heavy Head", "Roadrunner", "Ascend", "Flick",
    "Collapse", "Freeze", "High Gravity", "Midas Touch", "Rush", "Splatter", "Tremble", "Reaper", "Lost Soul", "Bonesplosion", "Frozen",
    "Gingerbreadify", "Bogey", "Decorated Player", "Snowmanify", "Warp Sickness", "Balloons", "BONK!", "Boogie", "Darkheart", "Rainbow Barf",
    "Clapped", "Enlightened", "Crushed", "GOOAAALLLL", "Chark Attack", "Diamond Hands", "Electrocute", "Heartbeat", "Ignite", "Low Gravity",
    "OOF", "Opulent", "Orbital Strike", "Pixel Coins", "Stiff", "Tough Crowd", "Zombified", "RIP", "Disintegrate", "Broom Ride", "Snowballed",
    "Falling Icicles", "David", "Giant Ice Spike", "DRIP", "Beacon", "Erased", "Firework Show", "Giant Snowball", "Impaled", "Sleigh Away",
    "Spaghettified", "Those Who Know", "Jolly Judgement", "Director's Cut", "Elfify", "5B Visits", "Neon Explosion", "Plasma Overload",
    "Void Corruption", "Frostbite", "Magma Eruption", "Galactic Collapse", "Holographic Glitch", "Chrono Freeze", "Rift Teleport", "Rainbow Barrage"
}
...
local function robust_require(module)
    ...
    pcall(function()
        local setidentity = setthreadidentity or set_thread_identity or (syn and syn.set_thread_identity)
        if setidentity and old_id then setidentity(old_id) end
    end)
            ...
            if ok3 and type(objs) == "table" then
                for _, v in pairs(objs) do
                    if type(v) == "table" then
                        if mName:find("CosmeticLibrary") and (v.Cosmetics or rawget(v, "Cosmetics")) and
                            ((type(v.Equip) == "function") or (type(v.GetSkins) == "function")) then
                            return v
                        end
                        if mName:find("ItemLibrary") and (v.ViewModels or rawget(v, "ViewModels")) then return v end
                        if mName:find("ClientViewModel") and (v.new or rawget(v, "new")) and
                            (v.GetWrap or rawget(v, "GetWrap")) then
                            return v
                        end
...
task.spawn(function()
    ...
    local success, err = pcall(function()
        ...
        local function getCosmeticData(name, cType)
            ...
            if not base and type(SC_Cosmetic) == "table" then
                for _, tbl in pairs(SC_Cosmetic) do
                    if type(tbl) == "table" and rawget(tbl, name) then base = tbl[name]; break end
                end
            end
        ...
        SC_ViewModel.new = function(replicatedData, clientItem)
            pcall(function()
                ...
                local charmKey, finisherKey
                pcall(function() charmKey = SC_RepClass:ToEnum("Charm") end)
                pcall(function() finisherKey = SC_RepClass:ToEnum("Finisher") end)
                ...
                if _G.EquippedData[wn] then
                    ...
                    if skin and skin ~= "Default" then
                        ...
                        if cd then
                            replicatedData[dk][skinKey] = cd
                            replicatedData[dk][nameKey] = skin
                        end
                    end
                end
                if charmKey and _G.GlobalData.Charm ~= "None" then
                    local cd = getCosmeticData(_G.GlobalData.Charm, "Charm")
                    if cd then replicatedData[dk][charmKey] = cd end
                end
                if finisherKey and _G.GlobalData.Finisher ~= "None" then
                    local cd = getCosmeticData(_G.GlobalData.Finisher, "Finisher")
        ...
        if isMobile then
            ...
            mobileBtn.Position = UDim2.new(1, -85, 1, -155)
            ...
            mobileBtn.MouseButton1Click:Connect(function() SC_Main.Visible = not SC_Main.Visible end)
        end
        ...
        local function SC_SetTab(tab)
            ...
            for _, pair in pairs({ { SkinsTabBtn, "Skins" }, { WrapsTabBtn, "Wraps" }, { CharmsTabBtn, "Charms" }, { FinishTabBtn, "Finish" } }) do
        ...
        buildListScroll(SC_CharmScroll, SC_CharmGrid, CharmList, function() return _G.GlobalData.Charm end, function(item)
            _G.GlobalData.Charm = item
            tryEquipGlobal("Charm", item)
            SC_SelLabel.Text = "✅  Charm: " .. item
        end)

        buildListScroll(SC_FinishScroll, SC_FinishGrid, FinisherList, function() return _G.GlobalData.Finisher end, function(item)
        ...
        local function SC_MakeWeaponBtn(weapon)
            ...
            local function UpdateBadges()
                ...
                skinBadge.Visible = skin ~= "Default"
                if skinBadge.Visible then skinBadge.Text = skin:sub(1, 7) end
                wrapBadge.Visible = wrap ~= "None"
                if wrapBadge.Visible then wrapBadge.Text = wrap:sub(1, 7) end
            end
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                for _, skin in ipairs(SkinLists[weapon]) do
                    ...
                    sb.BackgroundColor3 = (_G.EquippedData[weapon] and _G.EquippedData[weapon].Skin == skin) and Color3.fromRGB(0, 60, 50) or Color3.fromRGB(20, 20, 30)
                    sb.Image = ""
                    sb.BorderSizePixel = 0
                    sb.Parent = SC_SkinScroll
                    Instance.new("UICorner", sb).CornerRadius = UDim.new(0, 6)
                    local lbl = Instance.new("TextLabel", sb)
                    lbl.Size = UDim2.new(1, 0, 0, 38)
                    lbl.Position = UDim2.new(0, 0, 1, -38)
                ...
                buildListScroll(SC_WrapScroll, SC_WrapGrid, WrapList, function()
                    return (_G.EquippedData[weapon] and _G.EquippedData[weapon].Wrap) or "None"
                end, function(item)
    ...
    if not success then warn("[Skin Changer] Failed to load:", err) end
end)
