...
local function restoreAttribute(attribute)
    ...
    if type(cachedTables[attribute]) ~= "table" then
...
local function enableAntiKatana()
    katanaCached = gcCacheOnce({ "ReflectBullets", "CanReflect", "IsBlocking", "KatanaBlocking", "KatanaActive",
        "ParryActive", "BulletReflect", "Deflect", "IsParrying", "KatanaReflect", "ReflectDamage" })
...
local function disableAntiKatana()
    if antiKatanaConn then
        antiKatanaConn:Disconnect()
        antiKatanaConn = nil
    end
...
local function enableNoBounds()
    noBoundsCached = gcCacheOnce({ "OutOfBounds", "IsOutOfBounds", "OOB", "outOfBounds", "BoundsDead", "VoidKill",
        "InVoid", "FellOff", "KillOnFall", "DeathFloor", "KillFloor" })
...
local function disableNoBounds()
    if noBoundsConn then
        noBoundsConn:Disconnect()
        noBoundsConn = nil
    end
...
local function enableAlwaysBackstab()
    ...
    for _, e in pairs(backstabCached) do
        pcall(function()
            if type(e.tbl[e.key]) == "boolean" then
                e.tbl[e.key] = true
            elseif type(e.tbl[e.key]) == "number" then
                e.tbl[e.key] = 360
            end
        end)
    end
...
local function disableNoFireDamage()
    if fireDamageConn then
        fireDamageConn:Disconnect()
        fireDamageConn = nil
    end
...
local function createESP(player)
    ...
    espObjects[player] = {
        box       = newDrawing("Square",
            { Visible = false, Color = Color3.fromRGB(0, 255, 200), Thickness = 1.5, Filled = false, Transparency = 1 }),
        boxFill   = newDrawing("Square",
            { Visible = false, Color = Color3.fromRGB(0, 200, 150), Thickness = 1, Filled = true, Transparency = 0.9 }),
        ...
        healthBg  = newDrawing("Square",
            { Visible = false, Color = Color3.fromRGB(0, 0, 0), Thickness = 1, Filled = true, Transparency = 0.5 }),
        healthBar = newDrawing("Square",
            { Visible = false, Color = Color3.fromRGB(0, 255, 180), Thickness = 1, Filled = true, Transparency = 1 }),
        distance  = newDrawing("Text",
            {
                Visible = false,
                Color = Color3.fromRGB(180, 255, 240),
                Size = 11,
                Center = true,
                Outline = true,
                OutlineColor =
        ...
        if input.KeyCode == Enum.KeyCode.RightShift then
            ...
            if hidden then
                TweenService:Create(root, TweenInfo.new(0.25, Enum.EasingStyle.Quad),
                    { Position = UDim2.new(0, -560, 0.5, -190), BackgroundTransparency = 1 }):Play()
                ...
                TweenService:Create(root, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
                    { Position = UDim2.new(0.5, -270, 0.5, -190), BackgroundTransparency = 0 }):Play()
...
local function makeToggle(parent, labelText, key, order, onEnable, onDisable)
    ...
    btn.MouseEnter:Connect(function()
        TweenService:Create(row, TweenInfo.new(0.12),
            { BackgroundColor3 = Color3.fromRGB(18, 22, 28) }):Play()
    end)
    ...
    btn.MouseButton1Down:Connect(function()
        TweenService:Create(row, TweenInfo.new(0.08),
            { BackgroundColor3 = Color3.fromRGB(0, 25, 20) }):Play()
    end)
    ...
    local function updateVisual(on)
        TweenService:Create(pillBg, TweenInfo.new(0.18), { BackgroundColor3 = on and Color3.fromRGB(0, 35, 28) or C.bg3 })
            :Play()
...
local function makeButton(parent, labelText, order, onClick)
    ...
    btn.MouseButton1Down:Connect(function()
        TweenService:Create(btn, TweenInfo.new(0.08), { Size = UDim2.new(1, -4, 0, 28) })
            :Play()
    end)
    btn.MouseButton1Up:Connect(function()
        ...
        pb.MouseButton1Click:Connect(function()
            ...
            for _, o in pairs(options) do
                TweenService:Create(pickerBtns[o], TweenInfo.new(0.15), {
                    BackgroundColor3 = o == opt and C.neon2 or
                        C.bg3
                }):Play()
...
-- Forward declarations for config save/load (defined after skin data lists)
_G.EquippedData = _G.EquippedData or {}
_G.GlobalData = _G.GlobalData or { Charm = "None", Finisher = "None" }
local SC_SaveConfig, SC_LoadConfig
...
makeToggle(cL, "No Recoil", "NoRecoil", 2, function() applyAttribute("ShootRecoil", 0) end,
    function() restoreAttribute("ShootRecoil") end)
makeToggle(cL, "No Spread", "NoSpread", 3,
    function()
        applyAttribute("ShootSpread", 0)
        applyAttribute("ShootCooldown", 0)
    end, function()
        restoreAttribute("ShootSpread")
        restoreAttribute("ShootCooldown")
    end)
...
makeButton(cR, "⚡ Load Bolts Silent Aim", 2, function(btn, bs)
    ...
    local ok = pcall(function()
        loadstring(game:HttpGet(
            "https://raw.githubusercontent.com/ThunderScriptSolutions/Misc/refs/heads/main/RivalsSilentAim"))()
    end)
    if ok then
        btn.Text = "✓ Loaded!"
...
cfgStatus.Text = "💾  Config auto-loads on start"
...
-- Auto-load immediately on script start (runs before skin changer modules load,
-- so _G.EquippedData and _G.GlobalData are populated in time for the hooks)
SC_LoadConfig()

-- Auto-save when the player leaves the game (executor supports BindToClose on most)
pcall(function()
...
-- Auto-save every 60 seconds in the background so progress is never fully lost
task.spawn(function()
    while true do
        task.wait(60)
        pcall(SC_SaveConfig)
...
-- Auto-save when the character is removed (covers leaving a match / teleport)
plr.CharacterRemoving:Connect(function()
    pcall(SC_SaveConfig)
end)

-- Auto-load + re-apply cosmetics when character is added (respawn / rejoin)
-- The skin changer hooks handle visual injection automatically, but we also
-- re-call SC_Cosmetic.Equip for any already-running session.
plr.CharacterAdded:Connect(function()
    pcall(SC_LoadConfig)
    -- Re-apply is handled server-side by the SVM.new hook on next weapon equip.
    -- We print a note so the player knows it happened.
    task.wait(1)
    print("[CyberCheat] Config auto-loaded on respawn.")
end)
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
        ...
        root.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale,
            ...
            if ok3 and type(objs) == "table" then
                for _, v in pairs(objs) do
                    if type(v) == "table" then
                        ...
                        if mName:find("ClientViewModel") and (v.new or rawget(v, "new")) and (v.GetWrap or rawget(v, "GetWrap")) then
                            return
                                v
                        end
...
task.spawn(function()
    ...
    local SC_ViewModel = robust_require(SC_Modules:WaitForChild("ClientReplicatedClasses", 15):WaitForChild(
        "ClientFighter", 15):WaitForChild("ClientItem", 15):WaitForChild("ClientViewModel", 15))
    ...
    if isMobile then
        ...
        task.spawn(function()
            while mobileBtn.Parent do
                TweenService:Create(mobileBtn, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                    { BackgroundColor3 = Color3.fromRGB(0, 120, 100) }):Play()
                task.wait(0.7)
                TweenService:Create(mobileBtn, TweenInfo.new(0.7, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut),
                    { BackgroundColor3 = Color3.fromRGB(0, 180, 140) }):Play()
                task.wait(0.7)
            end
        end)
    ...
    local function SC_MakeWeaponBtn(weapon)
        ...
        local function UpdateBadges()
            ...
            if skin ~= "Default" then
                skinBadge.Text = skin:sub(1, 7)
                skinBadge.Visible = true
            else
                skinBadge.Visible = false
            end
            if wrap ~= "None" then
                wrapBadge.Text = wrap:sub(1, 7)
                wrapBadge.Visible = true
            else
                wrapBadge.Visible = false
            end
        end
        UpdateBadges()
    ...
    do
        ...
        UserInputService.InputChanged:Connect(function(input)
            if sc_dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
                ...
                SC_Main.Position = UDim2.new(sc_startPos.X.Scale, sc_startPos.X.Offset + d.X, sc_startPos.Y.Scale,
                    sc_startPos.Y.Offset + d.Y)
            end
        end)
    end
